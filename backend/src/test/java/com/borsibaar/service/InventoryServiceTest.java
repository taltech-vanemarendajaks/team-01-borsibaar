...
class InventoryServiceTest {
    ...
    void addStock_CreatesInventoryIfMissing() {
        ...
        when(inventoryRepository.findByProduct_OrganizationIdAndProduct_Id(1L, 5L)).thenReturn(Optional.empty());
    ...
    void removeStock_Insufficient_ThrowsBadRequest() {
        ...
        when(productRepository.findById(5L)).thenReturn(Optional.of(product));
        when(inventoryRepository.findByProduct_OrganizationIdAndProduct_Id(1L, 5L)).thenReturn(Optional.of(inv));
    ...
    void getByOrganization_FiltersInactiveProducts() {
        Product p1 = new Product();p1.setId(10L); p1.setActive(true); p1.setName("A");
        Inventory inv1 = new Inventory();inv1.setProduct(p1);inv1.setId(1L);inv1.setQuantity(BigDecimal.ONE);inv1.setUpdatedAt(OffsetDateTime.now());

        Product p2 = new Product();p2.setId(11L); p2.setActive(false); p2.setName("B");
        Inventory inv2 = new Inventory();inv2.setProduct(p2);inv2.setId(2L);inv2.setQuantity(BigDecimal.ONE);inv2.setUpdatedAt(OffsetDateTime.now());
        ...
        assertEquals("A", result.getFirst().productName());
    }

    @Test
    void getByProductAndOrganization_ProductInactive_Gone() {
        Inventory inv = new Inventory(); inv.setId(1L); inv.setQuantity(BigDecimal.ONE); inv.setUpdatedAt(OffsetDateTime.now());
        when(inventoryRepository.findByProduct_OrganizationIdAndProduct_Id(1L, 10L)).thenReturn(Optional.of(inv));
